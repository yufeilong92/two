...
class DriverListActivity : BaseActivity() {
    ...
    private var mAdapter: DriverAdapter? = null
    ...
    private fun initView() {
        //返回按钮
        driver_list_back.setOnClickListener { finishBase() }
        //设置编辑框弹出键盘右下角市搜索按钮
        driver_list_edit.setOnEditorActionListener { v, actionId, event ->
            when (actionId) {
                EditorInfo.IME_ACTION_SEARCH -> {
                    val phoneStr = driver_list_edit.text.toString().trim()
                    if (phoneStr.isEmpty()) {
                        ifLoading = true
                        toGetData(0)
                    } else {
        ...
        driver_list_edit.addTextChangedListener(object : TextWatcher {
        ...
        //初始化下拉刷新
        driver_list_refresh.apply {
            setOnRefreshListener { refreshlayout ->
                if (!ifSearch) {
                    toGetData(0)
                } else {
                    mViewModel.getCarOwnerByPhone(driver_list_edit.text.toString().trim())
                }
            }
        ...
        driver_list_recycle.layoutManager = LinearLayoutManager(mContext)
        driver_list_search.setOnClickListener {
            routerTo.jumpToDriverSearch()
        }

        //打电话
        item_driver_button02.setOnClickListener { routerTo.callPhone(driver_list_edit.text.toString().trim()) }
        //发短信
        item_driver_button01.setOnClickListener {
            ifLoading = true
            mViewModel.sendInvitationSms(driver_list_edit.text.toString().trim()) }
    ...
    private fun initViewModel() {
        mViewModel.isShowProgress.observe(this, Observer<Int> {
            when (it) {
                ...
                1 -> {
                    dismissProgress()
                    driver_list_refresh.finishLoadMore()
                    driver_list_refresh.finishRefresh()
                }
            }
        })
        //指定司机列表数据返回监听
        mViewModel.driverInfos.observe(this, Observer {
            ifLoading = false
            driver_list_refresh.visibility = View.VISIBLE
            if (it!!.pageInfo.page == 0) {
                infoList.clear()
            }
            if (it!!.pageInfo.last) {
                driver_list_refresh.setEnableLoadMore(false)
            } else {
                driver_list_refresh.setEnableLoadMore(true)
            }
        ...
        mViewModel.infos.observe(this, Observer {
            ...
            initAdapter()
            if (infoList.size == 0) {
                driver_contentno.visibility = View.VISIBLE
                driver_list_refresh.visibility = View.GONE
            } else {
                driver_contentno.visibility = View.GONE
                driver_list_refresh.visibility = View.VISIBLE
            }
            driver_list_refresh.setEnableLoadMore(false)
        })
...
//        mViewModel.getFamiliarCarList(pageNo)
    ...
    private fun initAdapter() {
        if (mAdapter == null) {
            driver_list_recycle.adapter = DriverAdapter(mContext!!, infoList, 1).apply {
                setOnItemClickListener { adapter, view, position ->

                }
                setOnItemChildClickListener { adapter, view, position ->
                    when (view.id) {
                        R.id.item_driver_button1 -> {
                            when (entryTemp) {
                                0 -> {
                                    ifLoading = true
                                    toCheckInfo {
                ...
                setEmptyView(R.layout.cell_empty, driver_list_recycle)
                mAdapter = this
            }
        } else {
