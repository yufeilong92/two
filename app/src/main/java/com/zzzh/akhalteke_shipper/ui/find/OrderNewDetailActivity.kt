...
class OrderNewDetailActivity : BaseActivity() {
    ...
    private lateinit var cancelDialog: OrderCancleDialog
...
//        gm_tv_activity_right_title.setOnClickListener { routerTo.callKeFu() }
        cancelDialog = object : OrderCancleDialog(mContext) {
            override fun cancelSubmit(content: String) {
                mViewModel.orderCancel(orderId, content)
            }
        }
    }
...
//            isShowTime(true)
        ...
        if (info.ifPay == "1") {
            ...
            tv_order_detail_pay_status.setOnClickListener {
                when (info.status) {
                    "1" -> {
                        //支付点击
                        pay(info)
        ...
        when (info.status) {
            "1" -> {//运输中
                ...
                when (info.ifAgreement) {//协议状态1待确认，2未发起，3同意，4拒绝
                    "1" -> {
                        ...
                        isShowSure(isShow = false, isShowStatus = true)
                    }
                    "2" -> {
                        isShowProtocol(false)
                        tv_order_detail_topro.setOnClickListener { routerTo.jumpToProtocolTo(orderInfo = info) }
                    ...
                    "3" -> {
                        ...
                        isShowSure(isShow = true, isShowStatus = false)
                    }
                    "4" -> {
                        isShowProtocol(true)
            ...
            "2" -> {//已完成
                isShowSure(isShow = false, isShowStatus = false)
                isShowProtocol(true)
                if (info.ifPay == "1") {
        ...
        tv_order_detail_cancel.setOnClickListener { ToolUtils.showDialog(cancelDialog) }
...
//                routerTo.jumpToOrderPay(info.orderId, info.totalMoney)
        ...
        tv_order_detail_server.setOnClickListener {
            val bundle = Bundle()
            bundle.putString(BaiDuMapActivity.DRIVER_ID, info.carOwnerId)
            bundle.putString(BaiDuMapActivity.PLATE_ID, info.carOwnerPlate)
            routerTo.jumpTo(BaiDuMapActivity::class.java, bundle)
        }
    }

    private fun pay(info: OrderInfo) {
        if (info.ifPay == "1") return
        //                routerTo.jumpToOrderPay(info.orderId, info.totalMoney)
        routerTo.jumpToOrderPay(
            info.loadAreaName,
            info.unloadAreaName,
     ...
     * 是否显示确认收货
     */
    private fun isShowSure(isShow: Boolean, isShowStatus: Boolean) {
        ll_order_detail_complate.visibility = if (isShow) View.VISIBLE else View.GONE
        ll_order_detail_bottom.visibility = if (isShowStatus) View.VISIBLE else View.GONE
    }
...
//            order_detail_to_label.visibility = View.GONE
    ...
    override fun onDestroy() {
        super.onDestroy()
        ToolUtils.dismissDialog(cancelDialog)
    }
