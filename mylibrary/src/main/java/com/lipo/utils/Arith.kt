...
object Arith { // 默认除法运算精度
    ...
    fun mul(v1: String, v2: Double): Double {
     ...
     * @return
     */
    fun removalzero(v: Double): String {
        val string = v.toString() + ""

        return if (string.indexOf(".") == string.length) {
            string.substring(0, string.indexOf("."))
        } else {
            if (string.endsWith(".0") || string.endsWith(".")
                    || string.endsWith(".00") || string.endsWith(".000")) {
                string.substring(0, string.indexOf("."))
            } else {
                BigDecimal(string).stripTrailingZeros().toString()
            }
        }
    }
}
